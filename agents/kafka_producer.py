...
async def publish_critical_event(
    ...
    Args:
        trigger_id: server.py가 이미 save_log_analysis()로 저장한 레코드 id.
                    consumer가 중복 저장 없이 이 id를 재사용한다.
    Returns:
        성공 여부 (Kafka 발행 기준)
    """
    ...
    message = {
        ...
        "trigger_id": trigger_id,   # server.py가 저장한 log_analysis.id — consumer 중복 저장 방지
