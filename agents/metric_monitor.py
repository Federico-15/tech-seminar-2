...
from .llm_router import call_llm, call_llm_local_4b, parse_json_response
...
async def analyze_metrics(metrics: dict) -> AnomalyResult:
    ...
    try:
        # 메트릭은 내부 데이터 — qwen3:4b(vLLM)로 직접 처리
        response, model_used = call_llm_local_4b(prompt=prompt, system=SYSTEM_PROMPT)
