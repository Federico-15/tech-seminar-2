...
async def fetch_recent_logs(minutes: int = 5, limit: int = 100, since: datetime | None = None) -> list[str]:
    """
    Loki에서 로그를 가져와 문자열 리스트로 반환합니다.

    Args:
        since: 이 시점 이후 로그만 가져옴 (None이면 최근 minutes분)
    Returns:
        로그 라인 문자열 리스트 (오래된 것 → 최신 순)
    """
    now = datetime.now(timezone.utc)
    start = since if since else now - timedelta(minutes=minutes)
